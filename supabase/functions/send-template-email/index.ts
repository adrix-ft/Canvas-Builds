...
serve(async (req) => {
  try {
    ...
    // Modern HTML Email Template (Your Preferred Design + Unsubscribe Link)
    const emailHTML = `
    <!DOCTYPE html>
    <html>
    <head>
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 24px; overflow: hidden; box-shadow: 0 10px 25px rgba(0,0,0,0.05);">
        ...
        <tr>
          <td style="background-color: #f8fafc; padding: 32px 40px; text-align: center; border-top: 1px solid #e2e8f0;">
            ...
            <p style="margin: 0 0 16px; font-size: 12px; color: #a0aec0; line-height: 1.5;">&copy; ${new Date().getFullYear()} Canvas Builds. All rights reserved.<br>Based in India, built with ❤️</p>
            <a href="https://canvas-builds.vercel.app/unsubscribe" style="color: #a0aec0; font-size: 12px; text-decoration: underline;">Unsubscribe from these emails</a>
          </td>
        </tr>
      </table>
    </body>
    ...
    // Chunking the emails into batches of 90 to respect Gmail limits and prevent timeouts
    const chunkSize = 90;
    const emailChunks = [];
    ...
    console.log(`Successfully sent emails in ${emailChunks.length} batch(es).`);

      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[60] bg-[#493129]/40 backdrop-blur-sm lg:hidden"
          ...
          >
            <motion.div 
              ...
              className="absolute right-0 top-0 bottom-0 w-[85%] max-w-sm bg-[#ffeddb] shadow-2xl flex flex-col rounded-l-[2rem] overflow-hidden"
              ...
              <div className="flex-1 overflow-y-auto py-6 px-4 flex flex-col gap-3">
                ...
                {['Home', 'Templates', 'Categories', 'Custom Request', 'Contact'].map((item, i) => (
                  <motion.a 
                    ...
                    key={i} href="#" 
                    className={\`flex items-center justify-between text-[15px] font-bold p-4 rounded-2xl transition-colors \${i === 0 ? 'bg-white text-[#efa3a0] shadow-sm' : 'text-[#493129]/80 hover:bg-white/50'}\`}
                  >
                    {item}
...
const Hero = () => {
  return (
    <div className="relative overflow-hidden">
      ...
      <div className="absolute bottom-10 right-4 sm:right-20 text-[#8b597b]/10 text-6xl sm:text-8xl blur-[2px] transform rotate-12 pointer-events-none">✨</div>
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-10 sm:pt-36 sm:pb-20 relative z-10">
        <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-20">
          ...
          <motion.div 
            ...
            initial="hidden"
            animate="show"
            className="flex-1 space-y-5 sm:space-y-6 w-full text-center lg:text-left z-10 pt-4"
          >
          ...
          <motion.div 
            ...
            className="flex-1 relative w-full max-w-sm sm:max-w-md mx-auto aspect-[4/3] mt-6 sm:mt-6"
          >
...
const TrustBadges = () => (
  <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
      <motion.div 
        ...
        variants={staggerContainer}
        className="bg-white/60 backdrop-blur-lg border border-white shadow-xl shadow-[#ffdec7]/30 rounded-[1.5rem] sm:rounded-[2.5rem] py-4 px-2 sm:p-8"
      >
...
const ProductCard = ({ product }: { product: typeof PRODUCTS[0] }) => (
  <motion.div variants={fadeUp} className="group cursor-pointer flex flex-col h-full bg-white/70 backdrop-blur-md rounded-[1.5rem] sm:rounded-[2rem] p-3 sm:p-4 shadow-md hover:shadow-xl hover:shadow-[#efa3a0]/20 transition-all duration-300 border border-white sm:hover:-translate-y-1">
     <div className={\`relative aspect-[4/3] sm:aspect-video rounded-xl sm:rounded-2xl bg-gradient-to-br \${product.gradient} mb-3 sm:mb-4 flex items-center justify-center overflow-hidden border border-white/50\`}>
        ...
        <button className="absolute top-2 right-2 sm:top-3 sm:right-3 w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white/90 backdrop-blur-sm shadow-sm flex items-center justify-center text-[#493129]/40 hover:text-rose-500 hover:bg-white transition-colors z-30 border border-white">
...
const PopularProducts = () => (
  <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-20 relative">
      <div className="flex flex-row justify-between items-end mb-6 sm:mb-10 gap-4">
         <div>
            <div className="flex items-center gap-1.5 sm:gap-2 mb-1 sm:mb-2">
              <span className="w-5 sm:w-8 h-px bg-[#efa3a0]"></span>
              <span className="text-[#efa3a0] font-bold text-[9px] sm:text-xs tracking-widest uppercase">Trending</span>
            </div>
            <h2 className="text-2xl sm:text-4xl font-serif text-[#493129]">Popular Templates</h2>
         </div>
         <button className="text-[#493129] text-xs sm:text-base font-bold flex items-center gap-1 sm:gap-2 hover:bg-[#ffeddb] transition-colors group bg-white px-3 sm:px-5 py-1.5 sm:py-2.5 rounded-full shadow-sm border border-[#ffdec7]/50 shrink-0">
            View All <ArrowRight className="w-3 h-3 sm:w-4 sm:h-4 transform group-hover:translate-x-1 transition-transform"/>
         </button>
      </div>
      ...
      <motion.div 
        ...
        viewport={{ once: true, margin: "-50px" }}
        variants={staggerContainer}
        className="flex overflow-x-auto hide-scrollbar sm:grid sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8 -mx-4 px-4 sm:mx-0 sm:px-0 snap-x snap-mandatory pb-4 sm:pb-0"
      >
         {PRODUCTS.map(p => (
            <div key={p.id} className="w-[80vw] sm:w-auto shrink-0 snap-center">
  ...
  <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8">
      ...
      <motion.div 
        ...
        className="bg-gradient-to-br from-[#ffdec7] to-[#ffeddb] rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-12 flex flex-col justify-center relative overflow-hidden group min-h-[260px] sm:min-h-[380px] shadow-lg shadow-[#ffdec7]/30 border border-white"
      >
      ...
      <motion.div 
        ...
        className="bg-gradient-to-br from-[#efa3a0]/30 to-[#8b597b]/10 rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-12 flex flex-col justify-center relative overflow-hidden group min-h-[260px] sm:min-h-[380px] shadow-lg shadow-[#efa3a0]/10 border border-white"
      >
        <div className="relative z-10 w-2/3 sm:w-2/3">
          <div className="inline-flex items-center gap-1.5 sm:gap-2 bg-white/80 backdrop-blur-sm px-2.5 sm:px-3 py-1 rounded-full border border-white mb-3 sm:mb-5">
  ...
  <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-20 relative">
      ...
      <div className="text-center mb-8 sm:mb-14">
          <div className="flex items-center justify-center gap-1.5 sm:gap-2 mb-2 sm:mb-3">
            <span className="w-4 sm:w-8 h-px bg-[#efa3a0]"></span>
            <span className="text-[#efa3a0] font-bold text-[9px] sm:text-xs tracking-widest uppercase">Our Value</span>
            <span className="w-4 sm:w-8 h-px bg-[#efa3a0]"></span>
          </div>
          <h2 className="text-2xl sm:text-4xl font-serif text-[#493129]">Why Buy Our Templates?</h2>
      </div>
      <motion.div 
        initial="hidden"
        whileInView="show"
        ...
        <div className="px-5 py-10 sm:py-12 md:px-16 md:py-20 flex flex-col lg:flex-row justify-between items-center gap-6 sm:gap-10 border-b border-white/10 relative z-10">
           ...
           <div className="flex flex-col sm:flex-row w-full lg:w-auto gap-2 sm:gap-0 relative z-10 p-1 sm:bg-white/10 sm:backdrop-blur-xl sm:rounded-full sm:border sm:border-white/20 mt-2 sm:mt-0">
              <input 
                ...
                className="px-5 sm:px-8 py-3.5 sm:py-5 bg-white/10 sm:bg-transparent border border-white/20 sm:border-none text-white w-full lg:w-80 rounded-[1rem] sm:rounded-none focus:outline-none placeholder:text-white/50 text-[13px] sm:text-base" 
              />
